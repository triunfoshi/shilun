...
class UIRouteTests(unittest.TestCase):
    def test_control_center_page_is_available(self) -> None:
        ...
        self.assertIn('id="analyzeResult" class="stock-output"', text)
        self.assertIn('<div class="card wide">\n        <h2>单票分析</h2>', text)
        self.assertIn('<div class="card wide">\n        <h2>每日日报 / 飞书</h2>', text)
        self.assertIn('class="card wide story-aligned-card" id="candidatesCard"', text)
